...
def log_validation(flux_transformer, args, accelerator, weight_dtype, step, is_final_validation=False, text_encoder=None, tokenizer=None):
    ...
    # Check if validation prompt file is specified
    if args.validation_prompt_file is not None and os.path.exists(args.validation_prompt_file):
        with open(args.validation_prompt_file, 'r') as f:
            validation_prompts = json.load(f)
        logger.info(f"Loaded {len(validation_prompts)} prompts from {args.validation_prompt_file}")
    else:
        validation_prompts = args.validation_prompt
        if isinstance(validation_prompts, str):
            validation_prompts = [validation_prompts]

    # Create output directory for this validation step
    validation_dir = os.path.join(args.output_dir, "validation", f"step_{step:06d}")
    ...
    try:
        for i, validation_prompt in enumerate(validation_prompts):
            images = []
            prompt_dir = os.path.join(validation_dir, f"prompt_{i:03d}")
            ...
            num_images = args.num_validation_images_per_prompt if hasattr(args, 'num_validation_images_per_prompt') else args.num_validation_images
            ...
            for j in range(num_images):
                try:
                    ...
                    if args.seed is not None:
                        img_generator = torch.Generator(device=accelerator.device).manual_seed(args.seed + j)
                    else:
                        img_generator = None
                        
                    with autocast_ctx:
                        torch.cuda.empty_cache()  # Free up memory before generation
                        image = pipeline(
                    ...
                    # Save image
                    image_path = os.path.join(prompt_dir, f"image_{j:02d}.png")
                    image.save(image_path)
                    
                    images.append(image)
                    logger.info(f"Generated image {j+1}/{num_images} for prompt {i+1}/{len(validation_prompts)}")
                    
                except Exception as e:
                    logger.error(f"Error generating image {j} for prompt '{validation_prompt}': {str(e)}")
                    continue
        ...
        # Always clean up to ensure memory is freed
        del pipeline
...
These are FLUX weights trained on {base_model}.
{img_str}
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a JSON file containing validation prompts. Takes precedence over validation_prompt if provided.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of images to be generated for each validation prompt when using validation_prompt_file",
    )
    parser.add_argument(
        "--validation_steps",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            'The transformer modules to apply LoRA training on. Please specify the layers in a comma separated string. '
            'E.g. - "to_k,to_q,to_v,to_out.0" will result in lora training of attention layers only, '
            '"ff.net.0.proj,ff.net.2" will train only FFN layers'
        ),
    )
    parser.add_argument(
        "--use_custom_tokens",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to load/save token embeddings",
    )
    parser.add_argument(
        "--save_token_embeddings",
        action="store_true",
        help="Whether to save token embeddings after training",
    )
...
def main(args):
    ...
    logger.info("loading flux_transformer")
    ...
    # Load text encoding pipeline for handling custom tokens
    text_encoding_pipeline = FluxPipeline.from_pretrained(
        args.pretrained_model_name_or_path, 
        transformer=None, 
        vae=None, 
        torch_dtype=torch.float32,  # Use float32 for precision in token embedding initialization
    )
    
    # Add custom tokens if requested
    custom_token_ids = None
    if args.use_custom_tokens:
        if args.token_embeddings_file and os.path.exists(args.token_embeddings_file):
            # Load existing token embeddings
            logger.info(f"Loading token embeddings from {args.token_embeddings_file}")
            text_encoding_pipeline, custom_token_ids = token_utils.load_token_embeddings(
                text_encoding_pipeline, args.token_embeddings_file
            )
        else:
            # Initialize new tokens
            logger.info(f"Setting up custom tokens: {args.custom_tokens} initialized with: {args.token_init_words}")
            text_encoding_pipeline, custom_token_ids = token_utils.setup_custom_tokens(
    ...
    # Move tokenizer and text encoder settings to the main pipeline now that custom tokens are added
    if args.use_custom_tokens:
        logger.info("Updating tokenizer and text encoder for the main pipeline")
        transformer_tokenizer = text_encoding_pipeline.tokenizer
        transformer_text_encoder = text_encoding_pipeline.text_encoder
    else:
        transformer_tokenizer = None
        transformer_text_encoder = None
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                torch.save(transformer_lora_layers_to_save, os.path.join(output_dir, "lora_weights.pt"))

                # Save token embeddings if requested
                if args.use_custom_tokens and args.save_token_embeddings:
                    token_embeddings_path = os.path.join(output_dir, "token_embeddings.pt")
                    logger.info(f"Saving token embeddings to {token_embeddings_path}")
                    token_utils.save_token_embeddings(
                        text_encoding_pipeline, custom_token_ids, token_embeddings_path
                    )
        ...
        def load_model_hook(models, input_dir):
            ...
            # 加载LoRA权重
            lora_state_dict = torch.load(os.path.join(input_dir, "lora_weights.pt"))
            incompatible_keys = set_peft_model_state_dict(
                transformer_, lora_state_dict, adapter_name="default"
            )
    ...
    # 添加以下代码来统计和打印参数：
    trainable_params = sum(p.numel() for p in transformer_lora_parameters)
    ...
    if accelerator.is_main_process and args.report_to == "wandb" and args.log_dataset_samples:
        ...
        logged_artifacts = []
        for img, prompt in zip(formatted_images, all_prompts):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    ...
                    if (args.validation_prompt is not None or args.validation_prompt_file is not None) and global_step % args.validation_steps == 0:
                        logger.info(f"Running validation at step {global_step}")
                        
                        # Make sure to free up memory before validation
                        torch.cuda.empty_cache()
                        ...
                        # Make sure to free memory after validation
                        torch.cuda.empty_cache()
    ...
    if accelerator.is_main_process:
        ...
        os.makedirs(args.output_dir, exist_ok=True)
        torch.save(transformer_lora_layers, os.path.join(args.output_dir, "lora_weights.pt"))

        # Save token embeddings if requested
        if args.use_custom_tokens and args.save_token_embeddings:
            token_embeddings_path = os.path.join(args.output_dir, "token_embeddings.pt")
            logger.info(f"Saving token embeddings to {token_embeddings_path}")
            token_utils.save_token_embeddings(
                text_encoding_pipeline, custom_token_ids, token_embeddings_path
            )
        ...
        image_logs = None
        if args.validation_prompt is not None or args.validation_prompt_file is not None:
            logger.info("Running final validation")
            
            # Make sure memory is freed before validation
            torch.cuda.empty_cache()
            ...
            try:
                ...
                # Continue with the rest of the cleanup even if validation fails
